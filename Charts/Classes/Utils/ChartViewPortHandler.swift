...
open class ChartViewPortHandler: NSObject
{
    ...
    let rowCount: CGFloat = 26.0 // BBT 曲线页总计纵向 26 格，包括底部 横坐标轴
    
    // 背景色占 _content 20 格
    open func offsetFilledBottom(_ bottomLine: Int) -> CGFloat
    {
        if bottomLine > 0 {
            return _chartHeight - _contentRect.origin.y - _contentRect.size.height * 20 / rowCount
        }
        else {
            return _chartHeight - _contentRect.origin.y - _contentRect.size.height
        }
    }
    
    open func offsetFilledTop(_ bottomLine: Int) -> CGFloat
    {
        if bottomLine > 0 {
            return _contentRect.origin.y + _contentRect.size.height * 6 / rowCount
        }
        else {
            return _contentRect.origin.y
        }
    ...
    open func contentSpace(_ bottomLine: Int) -> CGFloat
    {
        if bottomLine > 0 {
            return _contentRect.origin.y + _contentRect.size.height / rowCount * (rowCount - CGFloat(bottomLine) - 1)
        } else {
            return _contentRect.origin.y + _contentRect.size.height
        }
    }
    
    open func contentSpaceLabel(_ bottomLine: Int) -> CGFloat
    {
        if bottomLine > 0 {
            return _contentRect.origin.y + _contentRect.size.height / rowCount * (rowCount - 5)
        } else {
            return _contentRect.origin.y + _contentRect.size.height
        }
    }
    
    open func contentSpaceBottom(_ bottomLine: Int) -> CGFloat
    {
        if bottomLine > 0 {
            return _contentRect.origin.y + _contentRect.size.height / rowCount * (rowCount - 4)
        } else {
            return _contentRect.origin.y + _contentRect.size.height
        }
    }
    
    open func contentRealBottom(_ bottomLine: Int) -> CGFloat
    {
        if bottomLine > 0 {
            return _contentRect.origin.y + _contentRect.size.height / rowCount * (rowCount - 1)
        } else {
            return _contentRect.origin.y + _contentRect.size.height
        }
    }
