...
//    internal var _fillMatrixValueToPx = CGAffineTransform.identity
    
    /// matrix for handling the different offsets of the chart
    internal var _matrixOffset = CGAffineTransform.identity
    
    internal var _fillMatrixOffset = CGAffineTransform.identity
...
//        var fillScaleY = (_viewPortHandler.contentHeight * 7.0 / 24.0)
//        print("width: \(_viewPortHandler.contentWidth), height: \(_viewPortHandler.contentHeight)")
...
//        if CGFloat.infinity == fillScaleY
//        {
//            fillScaleY = 0.0
//        }
...
//        _fillMatrixValueToPx = CGAffineTransform.identity
//        _fillMatrixValueToPx = _fillMatrixValueToPx.scaledBy(x: scaleX, y: -fillScaleY)
//        _fillMatrixValueToPx = _fillMatrixValueToPx.translatedBy(x: CGFloat(-chartXMin), y: CGFloat(-34.9))
    }

    /// Prepares the matrix that contains all offsets.
    open func prepareMatrixOffset(_ yAxis: ChartYAxis)
    {
        if (!yAxis.inverted)
        {
            ...
            _fillMatrixOffset = CGAffineTransform(translationX: _viewPortHandler.offsetLeft, y: _viewPortHandler.chartHeight - _viewPortHandler.offsetFilledBottom(yAxis.bottomLineCount))
//            _fillMatrixOffset = CGAffineTransform(scaleX: 1.0, y: -1.0)
//            _fillMatrixOffset = _fillMatrixOffset.translatedBy(x: _viewPortHandler.offsetLeft, y: _viewPortHandler.offsetFilledBottom)
        }
        else
        {
            ...
            _fillMatrixOffset = CGAffineTransform(scaleX: 1.0, y: -1.0)
            _fillMatrixOffset = _fillMatrixOffset.translatedBy(x: _viewPortHandler.offsetLeft, y: -_viewPortHandler.offsetFilledTop(yAxis.bottomLineCount))
        }
    }
    ...
    open var fillValueToPixelMatrix: CGAffineTransform
    {
        return
            _matrixValueToPx.concatenating(_viewPortHandler.touchMatrix
                ).concatenating(_fillMatrixOffset
        )
    }
