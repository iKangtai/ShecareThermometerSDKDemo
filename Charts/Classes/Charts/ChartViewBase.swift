...
public protocol ChartViewDelegate
{
    ...
    @objc optional func downSectionSelected()
    
    @objc optional func bubbleMarkerSelected()
    
    @objc optional func markerLinkTouched()
}
...
open class ChartViewBase: NSUIView, ChartDataProvider, ChartAnimatorDelegate
{
    ...
    @objc open var noDataTextDescription: String?
    ...
    @objc open var marker: ChartMarker?
    
    /// the view that represents the bubble marker
    @objc open var bubbleMarker: ChartMarker?
    
    /// array of Entry that reference the bubble marker in the chart
    @objc open var bubbles = [ChartDataEntry]()
    open var bubbleAreas = [CGRect]()
        ...
        if _data === nil
        {
            ...
            if hasText
            {
                ChartUtils.drawText(
                    ...
                    align: .center,
                    attributes: [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): infoFont, NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): infoTextColor]
                )
            ...
            {
                ChartUtils.drawText(context: context, text: noDataTextDescription!, point: CGPoint(x: frame.width / 2.0, y: y), align: .center, attributes: [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): infoFont, NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): infoTextColor])
            }
    ...
    open func downSectionTouched(callDelegate: Bool) {
        if (callDelegate && delegate != nil)
        {
            delegate!.downSectionSelected?()
        }
    }
    ...
    internal func drawMarkers(context: CGContext)
    {
        ...
        for i in 0 ..< _indicesToHighlight.count
        {
            ...
            if xIndex <= Int(deltaX) && xIndex <= Int(CGFloat(deltaX) * _animator.phaseX)
            {
                ...
                let y = pos.y - (markerSize.height + 4) <= 0.0 ? (markerSize.height + 4) - pos.y : 0
                let x = pos.x - markerSize.width * 0.5 <= 0.0 ? markerSize.width * 0.5 - pos.x : 0
                marker!.draw(context: context, point: CGPoint(x: pos.x + x, y: pos.y + y))
            }
        }
    }
    
    /// draws all Bubble Marker Views
    internal func drawBubbles(context: CGContext)
    {
        // if there is no marker view or drawing marker is disabled
        if (bubbles.isEmpty)
        {
        ...
        for i in 0 ..< bubbles.count
        {
            ...
            // callbacks to update the content
            bubbleMarker!.refreshContent(entry: e, highlight: highlight)

            let markerSize = bubbleMarker!.size
            let y = pos.y - (markerSize.height + 4) <= 0.0 ? (markerSize.height + 4) - pos.y : 0
            let x = pos.x - markerSize.width * 0.5 <= 0.0 ? markerSize.width * 0.5 - pos.x : 0
            bubbleMarker!.draw(context: context, point: CGPoint(x: pos.x + x, y: pos.y + y))

            if let marker = bubbleMarker as? BubbleMarker {
                bubbleAreas.append(marker.area)
            }
        }
    }
    
    /// - returns: the actual position in pixels of the MarkerView for the given Entry in the given DataSet.
    @objc open func getMarkerPosition(entry: ChartDataEntry, highlight: ChartHighlight) -> CGPoint
    {
    ...
    @objc open func setDescriptionTextPosition(x: CGFloat, y: CGFloat)
    {
