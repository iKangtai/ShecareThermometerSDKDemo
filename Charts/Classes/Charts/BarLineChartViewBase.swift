...
open class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    {
        _rightAxisTransformer.prepareMatrixOffset(_rightAxis)
        _leftAxisTransformer.prepareMatrixOffset(_leftAxis)
    }
...
//        _rightAxis?._defaultValueFormatter = _defaultValueFormatter
    ...
    @objc open override func getMarkerPosition(entry e: ChartDataEntry, highlight: ChartHighlight) -> CGPoint
    {
        ...
        var pt = CGPoint(x: xPos + 0.5, y: yPos * _animator.phaseY)
    ...
    {
        let touchedPoint = recognizer.location(in: self)
        let chartH = self.bounds.size.height
        
        if touchedPoint.y > chartH * 21.0 / 25.0 {
            self.downSectionTouched(callDelegate: true)
            return
        }
        ...
        if (recognizer.state == NSUIGestureRecognizerState.ended)
        {
            ...
            if markerLinkTouched(touchedPoint) {
                delegate!.markerLinkTouched?()
                return
            }
            if bubbleSelected(touchedPoint) {
                delegate!.bubbleMarkerSelected?()
                return
            }
            
            if !self.isHighLightPerTapEnabled { return }
            
            let h = getHighlightByTouchPoint(touchedPoint)
    ...
    /// 判断是否点击了 “预测不准” 气泡
    private func bubbleSelected(_ point: CGPoint) -> Bool {
//        guard let eValue = bubbles.first?.value else { return false }
//
//        let yValue = getYValueByTouchPoint(pt: point, axis: ChartYAxis.AxisDependency.left)
//        let minY = eValue > 90 ? CGFloat(eValue + 0.1) : CGFloat(eValue + 0.03) // 华氏度和摄氏度的范围不一样
//        let maxY = eValue > 90 ? CGFloat(eValue + 0.36) : CGFloat(eValue + 0.18)
//        if yValue >= minY && yValue <= maxY {
//            guard let xIndex = highlighter?.getXIndex(point.x) else { return false }
//            let idx = xIndex - 1
//            for e in bubbles {
//                if idx >= e.xIndex - 1 && idx <= e.xIndex + 1 {
//                    return true
//                }
//            }
//        }
        for rect in bubbleAreas {
    ...
    /// 判断是否点击了 “提示气泡” 里的超链接
    private func markerLinkTouched(_ point: CGPoint) -> Bool {
        // if there is no marker view or drawing marker is disabled
        if (!drawMarkers || !valuesToHighlight()) {
            return false
        }
        guard let marker = marker as? BalloonMarker else { return false }
        if marker.area.contains(point) {
            return point.y > marker.area.maxY - 30
        }
        return false
    }
   ...
   @discardableResult private func performPanChange(translation: CGPoint) -> Bool
    {
        ...
        if (delegate !== nil && matrix.tx != originalMatrix.tx)
        {
    ...
    @objc open func zoom(_ scaleX: CGFloat, scaleY: CGFloat)
    {
        let matrix = _viewPortHandler.zoom(scaleX: scaleX, scaleY: scaleY, x: center.x, y: center.y)
        _viewPortHandler.refresh(newMatrix: matrix, chart: self, invalidate: false)
    ...
    @objc open func moveViewToX(_ xIndex: CGFloat)
    {
    ...
    /// - parameter yValue:
    /// - parameter axis: - which axis should be used as a reference for the y-axis
    @objc open func moveViewToY(_ yValue: Double, axis: ChartYAxis.AxisDependency)
    {
    ...
    /// - parameter yValue:
    /// - parameter axis: - which axis should be used as a reference for the y-axis
    @objc open func moveViewTo(xIndex: CGFloat, yValue: Double, axis: ChartYAxis.AxisDependency)
    {
        let valsInView = getDeltaY(axis) / _viewPortHandler.scaleY
    ...
    @objc open func getDeltaY(_ axis: ChartYAxis.AxisDependency) -> CGFloat
    {
        if (axis == .left)
        {
    ...
    @objc open func getPosition(_ e: ChartDataEntry, axis: ChartYAxis.AxisDependency) -> CGPoint
    {
    ...
    @objc open func getHighlightByTouchPoint(_ pt: CGPoint) -> ChartHighlight?
    {
        if _data === nil
        {
    ...
    /// `getPixelsForValues(...)`.
    @objc open func getValueByTouchPoint(pt: CGPoint, axis: ChartYAxis.AxisDependency) -> CGPoint
    {
        var pt = pt
    ...
    @objc open func getPixelForValue(_ x: Double, y: Double, axis: ChartYAxis.AxisDependency) -> CGPoint
    {
    ...
    @objc open func getYValueByTouchPoint(pt: CGPoint, axis: ChartYAxis.AxisDependency) -> CGFloat
    {
        return getValueByTouchPoint(pt: pt, axis: axis).y
    }
    
    /// - returns: the Entry object displayed at the touched position of the chart
    @objc open func getEntryByTouchPoint(_ pt: CGPoint) -> ChartDataEntry!
    {
    ...
    @objc open func getDataSetByTouchPoint(_ pt: CGPoint) -> IBarLineScatterCandleBubbleChartDataSet!
    {
        let h = getHighlightByTouchPoint(pt)
        if (h !== nil)
        {
    ...
    @objc open var xAxisRenderer: ChartXAxisRenderer
    {
    ...
    @objc open var leftYAxisRenderer: ChartYAxisRenderer
    {
    ...
    @objc open var rightYAxisRenderer: ChartYAxisRenderer
    {
    ...
    @objc open var isAnyAxinverted: Bool
    {
    ...
    @objc open func setYAxisMinWidth(_ which: ChartYAxis.AxisDependency, width: CGFloat)
    {
        if (which == .left)
        {
    ...
    @objc open func getYAxisMinWidth(_ which: ChartYAxis.AxisDependency) -> CGFloat
    {
        if (which == .left)
        {
    ...
    @objc open func setYAxisMaxWidth(_ which: ChartYAxis.AxisDependency, width: CGFloat)
    {
        if (which == .left)
        {
    ...
    @objc open func getYAxisMaxWidth(_ which: ChartYAxis.AxisDependency) -> CGFloat
    {
        if (which == .left)
        {
    ...
    @objc open func getYAxisWidth(_ which: ChartYAxis.AxisDependency) -> CGFloat
    {
        if (which == .left)
        {
    ...
    /// backwards.
    @objc open func getTransformer(_ which: ChartYAxis.AxisDependency) -> ChartTransformer
    {
        if (which == .left)
        {
    ...
    @objc open var maxVisibleValueCount: Int
    {
        get
        {
    ...
    @objc open var highestVisibleXIndex: Int
    {
        var pt = CGPoint(
