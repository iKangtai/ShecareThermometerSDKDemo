...
open class ChartData: NSObject
{
    ...
    @objc public init(xVals: [NSObject]?, dataSets: [IChartDataSet]?)
    {
        super.init()
    ...
    @objc open func addXValue(_ xVal: String?)
    {
        _xVals.append(xVal)
    }
    
    /// Removes the x-value at the specified index.
    @objc open func removeXValue(_ index: Int)
    {
    ...
    /// - returns: the average length (in characters) across all values in the x-vals array
    @objc open var xValsObjc: [NSObject]
    {
        get
        {
