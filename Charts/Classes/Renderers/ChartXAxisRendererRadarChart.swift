...
open class ChartXAxisRendererRadarChart: ChartXAxisRenderer
{
    ...
    open override func renderAxisLabels(context: CGContext)
    {
		...
		for i in stride(from: 0, to: xAxis.values.count, by: modulus)
        {
            ...
            drawLabel(context: context, label: label!, xIndex: i, x: p.x, y: p.y - xAxis.labelRotatedHeight / 2.0, attributes: [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): labelFont, NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): labelTextColor], anchor: drawLabelAnchor, angleRadians: labelRotationAngleRadians)
        }
    }
