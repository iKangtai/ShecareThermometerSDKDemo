...
open class LineChartRenderer: LineRadarChartRenderer
{
    ...
    open func drawLinear(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        if (dataSet.colors.count > 1)
        {
			...
			for j in stride(from: minx, to: count, by: 1)
            {
                ...
                _lineSegments[0].x = CGFloat(e.xIndex) + (dataSet.offsetEnabeld ? 0.5 : 0.0)
                ...
                if (j + 1 < count)
                {
                    ...
                    {
                        _lineSegments[1] = CGPoint(x: CGFloat(e.xIndex) + (dataSet.offsetEnabeld ? 0.5 : 0.0),
                                                   y: CGFloat(e.value) * phaseY)
                    }
                }
                else
                {
            ...
            if e1 != nil
            {
				...
				for x in stride(from: (count > 1 ? minx + 1 : minx), to: count, by: 1)
                {
                    ...
                    _lineSegments[j] = CGPoint(
                           x: CGFloat(e1.xIndex) + (dataSet.offsetEnabeld ? 0.5 : 0.0),
                    ...
                    _lineSegments[j] = CGPoint(
                        x: CGFloat(e2.xIndex) + (dataSet.offsetEnabeld ? 0.5 : 0.0),
    ...
    open func drawLinearFill(context: CGContext, dataSet: ILineChartDataSet, minx: Int, maxx: Int, trans: ChartTransformer)
    {
        ...
        let filled = generateFilledPath(
            ...
            from: minx,
            to: maxx,
            matrix: trans.fillValueToPixelMatrix)
    ...
    private func drawCircles(context: CGContext)
    {
        ...
        for i in 0 ..< dataSets.count
        {
			...
			for j in stride(from: minx, to: Int(ceil(CGFloat(maxx - minx) * phaseX + CGFloat(minx))), by: 1)
            {
                ...
                if let eData = e.data as? [String: NSString] {
                    if eData.keys.contains("dao") {
                        let daoStr = eData["dao"]
                        let paragraphStyle = NSMutableParagraphStyle()
                        paragraphStyle.alignment = .center
                        
                        let cdColor = UIColor(red: 0xFF/255.0, green: 0x74/255.0, blue: 0x86/255.0, alpha: 1.0)
                        let attrs = [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 8),
                                     NSAttributedStringKey.paragraphStyle: paragraphStyle,
                                     NSAttributedStringKey.foregroundColor: cdColor]
                        daoStr!.draw(in: CGRect(x: pt.x - 8.0, y: pt.y - 6.0, width: circleDiameter, height: 10.0), withAttributes: attrs)
                    } else if eData.keys.contains("cd") {
                        let cdValue = eData["cd"]!
                        let str = "\(cdValue)" as NSString
                        let paragraphStyle = NSMutableParagraphStyle()
                        paragraphStyle.alignment = .center
                        
                        let cdColor = UIColor(red: 82.0/255.0, green: 79.0/255.0, blue: 79.0/255.0, alpha: 1.0)
                        let attrs = [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 8),
                                     NSAttributedStringKey.paragraphStyle: paragraphStyle,
                                     NSAttributedStringKey.foregroundColor: cdColor]
                        str.draw(in: CGRect(x: pt.x - 7.5, y: pt.y - 6.0, width: 15.0, height: 10.0), withAttributes: attrs)
                    } else if eData.keys.contains("sex") || eData.keys.contains("cm") || eData.keys.contains("lh") || eData.keys.contains("hcg") {
                        let resultStr = (eData.keys.contains("sex") ? eData["sex"]! : (eData.keys.contains("cm") ? eData["cm"]! : (eData.keys.contains("lh") ? eData["lh"]! : eData["hcg"]!))) as String
                        let imgW: CGFloat = UIScreen.main.bounds.size.width < UIScreen.main.bounds.size.height ? 15.0 : 9.0
                        if let srcImage = UIImage(named: resultStr) {
                            // Quartz重绘图片
                            ...
                            currentContext?.draw(srcImage.cgImage!, in: rect) // 绘图
                            ...
                            let resultImg = flipImage.cgImage!
                            context.draw(resultImg, in: CGRect(x: pt.x - imgW * 0.5, y: pt.y - imgW * 0.5, width: imgW, height: imgW))
                        }
                    }
                    continue  // ?
                }
                ...
                if drawTransparentCircleHole
                {
                    ...
                    // Cut hole in path
                    context.addArc(center: CGPoint(x: pt.x, y: pt.y), radius: circleHoleRadius, startAngle: 0.0, endAngle: CGFloat(.pi * 0.5), clockwise: true)
    ...
    open override func drawHighlighted(context: CGContext, indices: [ChartHighlight])
    {
        ...
        for high in indices
        {
            ...
            for dataSetIndex in minDataSetIndex..<maxDataSetIndex
            {
                ...
                _highlightPointBuffer.x = CGFloat(xIndex) + 0.5
