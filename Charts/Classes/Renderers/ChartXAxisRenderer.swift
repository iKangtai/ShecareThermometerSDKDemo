...
open class ChartXAxisRenderer: ChartAxisRendererBase
{
    ...
    open override func renderAxisLabels(context: CGContext)
    {
        ...
        {
            drawLabels(context: context, pos: viewPortHandler.contentSpace(xAxis.bottomLineCount) + yOffset, anchor: CGPoint(x: 0.5, y: 0.0))
        }
...
//            drawLabels(context: context, pos: viewPortHandler.contentBottom - yOffset - xAxis.labelRotatedHeight, anchor: CGPoint(x: 0.5, y: 0.0))
            drawLabels(context: context, pos: viewPortHandler.contentSpaceLabel(xAxis.bottomLineCount) + (UIScreen.main.bounds.size.width > UIScreen.main.bounds.size.height ? 2.0 : 0.0) - yOffset - xAxis.labelRotatedHeight, anchor: CGPoint(x: 0.5, y: 0.0))
        }
        else
        { // BOTH SIDED
            drawLabels(context: context, pos: viewPortHandler.contentTop - yOffset, anchor: CGPoint(x: 0.5, y: 1.0))
    ...
    open override func renderAxisLine(context: CGContext)
    {
        ...
        {
            _axisLineSegmentsBuffer[0].x = viewPortHandler.contentLeft
            _axisLineSegmentsBuffer[0].y = viewPortHandler.contentSpace(xAxis.bottomLineCount)
            _axisLineSegmentsBuffer[1].x = viewPortHandler.contentRight
            _axisLineSegmentsBuffer[1].y = viewPortHandler.contentSpace(xAxis.bottomLineCount)
            context.strokeLineSegments(between: _axisLineSegmentsBuffer)
        }
    ...
    open func drawLabels(context: CGContext, pos: CGFloat, anchor: CGPoint)
    {
        ...
        let labelAttrs: [NSAttributedStringKey : NSObject] = [
            NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): xAxis.labelFont,
            NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): xAxis.labelTextColor,
            NSAttributedStringKey(rawValue: NSAttributedStringKey.paragraphStyle.rawValue): paraStyle
        ]
        let todayLabelAttrs: [NSAttributedStringKey : NSObject] = [
            NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): xAxis.labelFont,
            NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): xAxis.labelTextSpecialColor,
            NSAttributedStringKey(rawValue: NSAttributedStringKey.paragraphStyle.rawValue): paraStyle
        ]
		...
		for i in stride(from: self.minX, to: min(self.maxX + 1, xAxis.values.count), by: xAxis.axisLabelModulus)
        {
            ...
            let formattedLabel = xAxis.valueFormatter?.stringForXValue(i, original: label!, viewPortHandler: viewPortHandler) ?? label
            let isToday = formattedLabel?.contains("今") == true
            let attrs = isToday ? todayLabelAttrs : labelAttrs
            ...
            if (viewPortHandler.isInBoundsX(position.x))
            {
                ...
                var offs = i == self.minX ? (CGPoint(x: CGFloat(i + xAxis.axisLabelModulus), y: 0.0).applying(valueToPixelMatrix).x - position.x) * 0.5
                    : (position.x - CGPoint(x: CGFloat(i - xAxis.axisLabelModulus), y: 0.0).applying(valueToPixelMatrix).x) * 0.5 + position.x
                ...
                if (xAxis.avoidFirstLastClippingEnabled)
                {
                    ...
                    {
                        let width = labelns.boundingRect(with: labelMaxSize, options: .usesLineFragmentOrigin, attributes: attrs, context: nil).size.width
                        
                        if (width > viewPortHandler.offsetRight * 2.0
                            && offs + width > viewPortHandler.chartWidth)
                        {
                            offs -= width / 2.0
                        }
                    }
                ...
                let posAjusted = UIScreen.main.bounds.size.width > UIScreen.main.bounds.size.height ? pos - 3 : pos + 5
                let x = xAxis.labelAlignedLineEnabled ? position.x : offs
                drawLabel(context: context, label: isToday ? String(label!.dropFirst()) : label!, xIndex: i, x: x, y: posAjusted, attributes: attrs, constrainedToSize: labelMaxSize, anchor: anchor, angleRadians: labelRotationAngleRadians)
            }
        }
    }
    
    open func drawLabel(context: CGContext, label: String, xIndex: Int, x: CGFloat, y: CGFloat, attributes: [NSAttributedStringKey: NSObject], constrainedToSize: CGSize, anchor: CGPoint, angleRadians: CGFloat)
    {
        guard let xAxis = xAxis else { return }
    ...
    open override func renderGridLines(context: CGContext)
    {
		...
		for i in stride(from: self.minX, to: self.maxX, by: xAxis.axisLabelModulus)
        {
                ...
                && position.x <= viewPortHandler.chartWidth)
            {
                ...
                _gridLineSegmentsBuffer[1].y = viewPortHandler.contentSpace(xAxis.bottomLineCount)
                context.strokeLineSegments(between: _gridLineSegmentsBuffer)
                
                _gridLineSegmentsBuffer[0].x = position.x
                _gridLineSegmentsBuffer[0].y = viewPortHandler.contentSpaceBottom(xAxis.bottomLineCount)
                _gridLineSegmentsBuffer[1].x = position.x
                _gridLineSegmentsBuffer[1].y = viewPortHandler.contentRealBottom(xAxis.bottomLineCount)
                context.strokeLineSegments(between: _gridLineSegmentsBuffer)
            }
        }
