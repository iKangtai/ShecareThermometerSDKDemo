...
open class ChartYAxisRenderer: ChartAxisRendererBase
{
    ...
    open override func renderAxisLine(context: CGContext)
    {
        ...
        if (yAxis.axisDependency == .left)
        {
            ...
            _axisLineSegmentsBuffer[1].y = viewPortHandler.contentSpace(yAxis.bottomLineCount)
    ...
    internal func drawYLabels(context: CGContext, fixedPosition: CGFloat, offset: CGFloat, textAlign: NSTextAlignment)
    {
        ...
        for i in 0 ..< yAxis.entryCount
        {
            ...
            pt.x = 0
            if (i < yAxis.bottomLineCount) {
                let nextEntry = (i >= yAxis.entryCount - 1) ? yAxis.axisMaxValue : yAxis.entries[i + 1]
                pt.y = CGFloat(yAxis.entries[i]) + CGFloat(nextEntry - yAxis.entries[i]) * 0.5
            } else if (i == yAxis.bottomLineCount && yAxis.bottomLineCount > 0) {
                let nextEntry = (i >= yAxis.entryCount - 1) ? yAxis.axisMaxValue : yAxis.entries[i + 1]
                pt.y = CGFloat(yAxis.entries[i]) + CGFloat(nextEntry - yAxis.entries[i]) * 0.25
            } else {
                pt.y = CGFloat(yAxis.entries[i])
            }
            pt = pt.applying(valueToPixelMatrix)
            
            pt.x = fixedPosition + (i <= yAxis.bottomLineCount ? 4 : 0)
            pt.y += offset
            
            let attrs = i < (yAxis.bottomLineCount - 1) ? [
                ...
                NSAttributedStringKey(rawValue: NSAttributedStringKey.underlineStyle.rawValue): NSNumber(value: NSUnderlineStyle.styleSingle.rawValue)
            ] : [
                NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): labelFont,
                NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): labelTextColor
            ]
            
            ChartUtils.drawText(context: context, text: text, point: pt, align: textAlign, attributes: attrs)
        }
    }
    ...
    open override func renderGridLines(context: CGContext)
    {
        ...
        if yAxis.drawGridLinesEnabled
        {
            ...
            {
                if i == 0 || i == yAxis.bottomLineCount { continue }
                position.x = 0.0
